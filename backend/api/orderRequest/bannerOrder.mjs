...
const bannerOrder = express.Router();

bannerOrder.get('/banners', async (req, res) => {
    try {
      ...
      const orders = await prisma.bannerOrder.findMany({
        where: {
          productId: {
            in: productIds,
          },
  ...
  bannerOrder.post('/banner', async (req, res) => {
  try {
    ...
    const newOrder = await prisma.bannerOrder.create({
      data: {
        userId,
        productId,
        fullName,
