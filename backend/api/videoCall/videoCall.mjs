...
const friendsVideo = express.Router();
...
friendsVideo.post('/start', async (req, res) => {
  try {
    ...
    activeMeetings.set(meetingLink, meetingWss);
...
friendsVideo.get('/:meetingLink', (req, res) => {
  const { meetingLink } = req.params;
  const meetingWss = activeMeetings.get(meetingLink);
...
function handleSignalingData(data, sender) {
  const meetingWss = activeMeetings.get(data.meetingLink);
...
async function handleParticipantLeaving(leavingParticipant) {
  activeMeetings.forEach(async (meetingWss) => {
