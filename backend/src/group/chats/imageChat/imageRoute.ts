...
const groupImageChatRouter = express.Router();

// Upload and send image message (Create)
groupImageChatRouter.post('/messages/:groupId/:userId', (req: Request, res: Response) => {
...
groupImageChatRouter.get('/messages/:groupId', async (req: Request, res: Response) => {
  try {
...
groupImageChatRouter.put('/messages/:messageId', (req: Request, res: Response) => {
  upload(req, res, async (err) => {
    if (err) {
      console.error('File upload error:', err);
      return res.status(500).send('Error uploading file.');
...
groupImageChatRouter.delete('/messages/:messageId', async (req: Request, res: Response) => {
  try {
    const { messageId } = req.params;
...
export default groupImageChatRouter;
