...
const groupRecordingChatRouter = express.Router();
...
groupRecordingChatRouter.post('/sendRecordingMessage/:groupId/:userId', upload.single('file'), async (req: Request, res: Response) => {
  try {
...
groupRecordingChatRouter.get('/getRecordingMessages/:groupId', async (req: Request, res: Response) => {
  const { groupId } = req.params;

  try {
    const messages = await prisma.groupChat.findMany({
      where: { groupId, type: ChatMessageType.RECORDING },
...
groupRecordingChatRouter.put('/updateRecordingMessage/:messageId', upload.single('file'), async (req: Request, res: Response) => {
  try {
...
groupRecordingChatRouter.delete('/deleteRecordingMessage/:messageId', async (req: Request, res: Response) => {
  const { messageId } = req.params;
...
export default groupRecordingChatRouter;
