...
const academyAdvanceClassImageChat = express.Router();

// Upload and send image message (Create)
academyAdvanceClassImageChat.post('/messages/:classId/:userId', (req: Request, res: Response) => {
...
academyAdvanceClassImageChat.get('/messages/:classId', async (req: Request, res: Response) => {
  try {
...
academyAdvanceClassImageChat.put('/messages/:messageId', (req: Request, res: Response) => {
  upload(req, res, async (err) => {
    if (err) {
      console.error('File upload error:', err);
      return res.status(500).send('Error uploading file.');
...
academyAdvanceClassImageChat.delete('/messages/:messageId', async (req: Request, res: Response) => {
  try {
    const { messageId } = req.params;
...
export default academyAdvanceClassImageChat;
