...
class ClassService {
  ...
  async createClass(name: string, description: string, iconFile: Buffer, mimeType: string): Promise<any> {
    try {
      ...
      const newClass = await prisma.academy.create({
        data: {
          name,
          description,
          icon: iconUrl,
  ...
  async updateClassIcon(academyId: string, iconFile: Buffer, mimeType: string): Promise<string> {
    try {
      ...
      const iconKey = `group-icons/${academyId}/${Date.now()}`;

      const iconUrl = await uploadImageToS3(processedImage, iconKey, mimeType);

      const updatedGroup = await prisma.academy.update({
        where: { id: academyId,
         },
        data: { icon: iconUrl },
      });
  ...
  async deleteClassIcon(academyId: string): Promise<void> {
    try {
     
      const classes = await prisma.academy.findUnique({
        where: { id: academyId },
        select: { icon: true },
      });
  ...
  async getClassIcon(academyId: string): Promise<string | null> {
    try {
      const classes = await prisma.academy.findUnique({
        where: { id: academyId },
        select: { icon: true },
