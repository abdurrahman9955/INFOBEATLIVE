...
export const startVideoCall = async (academyId: string, userId: string) => {
  const videoCall = await prisma.academyVideoCall.create({
    data: {
      academyId,
      userId,
      startedAt: new Date(),
...
export const endVideoCall = async (callId: string) => {
  const videoCall = await prisma.academyVideoCall.update({
    where: { id: callId },
...
export const joinVideoCall = async (callId: string, userId: string) => {
  const videoCallParticipant = await prisma.academyVideoCallParticipant.create({
    data: {
      callId,
      userId,
    },
...
export const leaveVideoCall = async (callId: string, userId: string) => {
  const videoCallParticipant = await prisma.academyVideoCallParticipant.deleteMany({
    where: { callId, userId },
  });
