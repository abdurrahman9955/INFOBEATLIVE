...
export const startVoiceCall = async (classId: string, userId: string) => {
  const voiceCall = await prisma.academyClassVideoCall.create({
    data: {
      classId,
      userId,
      level:'BEGINNER',
...
export const endVoiceCall = async (callId: string) => {
  const voiceCall = await prisma.academyClassVideoCall.update({
    where: { id: callId,
      level:'BEGINNER',
     },
...
export const joinVoiceCall = async (callId: string, userId: string) => {
  const voiceCallParticipant = await prisma.academyClassVideoCallParticipant.create({
    data: {
      callId,
      userId,
      level:'BEGINNER',
...
export const leaveVoiceCall = async (callId: string, userId: string) => {
  const voiceCallParticipant = await prisma.academyClassVideoCallParticipant.deleteMany({
    where: { callId, userId,
      level:'BEGINNER',
     },
  });
