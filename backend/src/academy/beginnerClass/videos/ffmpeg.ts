...
const processAndUploadVideo = async (
    ...
    if (existingThumbnailPath) {
        ...
        thumbnailPath = path.join(__dirname, `${fileName}_thumbnail.jpg`);
        await new Promise<void>((resolve, reject) => {
            ffmpeg(filePath)
                .screenshots({
                    count: 1,
    ...
    const processedThumbnailPath = path.join(__dirname, `${fileName}_processed_thumbnail.jpg`);
    await sharp(thumbnailPath)
        .resize(640, 360)
        .toFile(processedThumbnailPath);

    // Upload Thumbnail to S3
    const thumbnailStream = fs.createReadStream(processedThumbnailPath);
    await uploadToS3(process.env.AWS_BUCKET!, `thumbnails/${fileName}_thumbnail.jpg`, thumbnailStream, 'image/jpeg');
    fs.unlinkSync(thumbnailPath);
    fs.unlinkSync(processedThumbnailPath);
