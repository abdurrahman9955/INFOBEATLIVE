...
// Initialize router
const academyIntermediateClassDocumentChat = Router();

// Define allowed document types
const allowedFileTypes = ['application/pdf', 'application/msword', 'application/vnd.openxmlformats-officedocument.wordprocessingml.document'];

// Handle uploading a document
academyIntermediateClassDocumentChat.post('/uploadDocument/:classId/:userId', upload, async (req: Request, res: Response) => {
  try {
    if (!req.file) {
...
academyIntermediateClassDocumentChat.get('/getMessages/:classId', async (req: Request, res: Response) => {
  try {
...
academyIntermediateClassDocumentChat.put('/updateDocument/:messageId', upload, async (req: Request, res: Response) => {
  try {
...
academyIntermediateClassDocumentChat.delete('/deleteDocument/:messageId', async (req: Request, res: Response) => {
  try {
    const { messageId } = req.params;

    // Find the message to be deleted
    const message = await prisma.academyClassChat.findUnique({ where: { id: messageId,
      level:'INTERMEDIATE',
     } });
...
export default academyIntermediateClassDocumentChat;
