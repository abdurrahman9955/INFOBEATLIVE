...
const academyIntermediateClassImageChat = express.Router();

// Upload and send image message (Create)
academyIntermediateClassImageChat.post('/messages/:classId/:userId', (req: Request, res: Response) => {
...
academyIntermediateClassImageChat.get('/messages/:classId', async (req: Request, res: Response) => {
  try {
...
academyIntermediateClassImageChat.put('/messages/:messageId', (req: Request, res: Response) => {
  upload(req, res, async (err) => {
    if (err) {
      console.error('File upload error:', err);
      return res.status(500).send('Error uploading file.');
...
academyIntermediateClassImageChat.delete('/messages/:messageId', async (req: Request, res: Response) => {
  try {
    const { messageId } = req.params;
...
export default academyIntermediateClassImageChat;
