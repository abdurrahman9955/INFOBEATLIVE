...
class ClassService {
  ...
  async createClass(name: string, description: string, iconFile: Buffer, mimeType: string): Promise<any> {
    try {
      ...
      const newClass = await prisma.bootcampClass.create({
        data: {
          name,
          description,
          icon: iconUrl,
  ...
  async updateClassIcon(classId: string, iconFile: Buffer, mimeType: string): Promise<string> {
    try {
      ...
      const updatedGroup = await prisma.bootcampClass.update({
        where: { id: classId,
            level: 'BEGINNER',
         },
        data: { icon: iconUrl },
  ...
  async deleteClassIcon(classId: string): Promise<void> {
    try {
      ...
      const classes = await prisma.bootcampClass.findUnique({
        where: { id: classId },
        select: { icon: true },
      });
      ...
      await prisma.bootcampClass.update({
        where: { id: classId },
        data: { icon: null },
      });
    } catch (error) {
