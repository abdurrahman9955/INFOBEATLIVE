...
// Initialize router
const bootcampIntermediateClassDocumentChat = Router();

// Define allowed document types
const allowedFileTypes = ['application/pdf', 'application/msword', 'application/vnd.openxmlformats-officedocument.wordprocessingml.document'];

// Handle uploading a document
bootcampIntermediateClassDocumentChat.post('/uploadDocument/:classId/:userId', upload, async (req: Request, res: Response) => {
  try {
    if (!req.file) {
...
bootcampIntermediateClassDocumentChat.get('/getMessages/:classId', async (req: Request, res: Response) => {
  try {
...
bootcampIntermediateClassDocumentChat.put('/updateDocument/:messageId', upload, async (req: Request, res: Response) => {
  try {
...
bootcampIntermediateClassDocumentChat.delete('/deleteDocument/:messageId', async (req: Request, res: Response) => {
  try {
    const { messageId } = req.params;

    // Find the message to be deleted
    const message = await prisma.bootcampClassChat.findUnique({ where: { id: messageId,
      level:'INTERMEDIATE',
     } });
...
export default bootcampIntermediateClassDocumentChat;
