...
videoPost.use(express.json());

// Upload Route
videoPost.post('/upload', upload.single('video'), async (req: UploadRequest, res: Response) => {
    try {
        ...
        await saveMediaRecord( userId, contentUrl, thumbnailUrl, title, description);
...
videoPost.get('/media/post', async (req: Request, res: Response) => {
    try {

        const mediaList = await prisma.post.findMany({
            where: {
                 type:'VIDEO',
                },
        });
...
videoPost.get('/media/group/:groupId/user/:userId', async (req: Request, res: Response) => {
    try {
...
// Get Media Route
videoPost.get('/media/:id', async (req: Request, res: Response) => {
    try {
...
// Delete Media Route
videoPost.delete('/media/:id', async (req: Request, res: Response) => {
    try {
        const media = await prisma.post.delete({
            where: {
                type: 'VIDEO',
                 id: req.params.id
                 },
...
// Update Media Route
videoPost.put('/media/:id', async (req: Request, res: Response) => {
    try {
        const { title, description } = req.body;

        const media = await prisma.post.update({
            where: {
                 type:'VIDEO',
                 id: req.params.id
                },
