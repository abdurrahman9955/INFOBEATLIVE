...
const classImageChatRouter = express.Router();

// Upload and send image message (Create)
classImageChatRouter.post('/messages/:classId/:userId', (req: Request, res: Response) => {
...
classImageChatRouter.get('/messages/:classId', async (req: Request, res: Response) => {
  try {
...
classImageChatRouter.put('/messages/:messageId', (req: Request, res: Response) => {
  upload(req, res, async (err) => {
    if (err) {
      console.error('File upload error:', err);
      return res.status(500).send('Error uploading file.');
...
classImageChatRouter.delete('/messages/:messageId', async (req: Request, res: Response) => {
  try {
    const { messageId } = req.params;

    // Find the message to be deleted
    const message = await prisma.classChat.findUnique({
      where: { id: messageId },
    });
...
export default classImageChatRouter;
