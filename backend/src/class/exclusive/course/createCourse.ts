...
export const createCourse = async (req: Request, res: Response) => {
  ...
  try {
    ...
    const course = await prisma.exclusive.create({
      data: {
        title,
        description,
        category,
...
export const getAllCourses = async (req: Request, res: Response) => {
  try {
    const courses = await prisma.exclusive.findMany({
      include: {
        sections: true,
      },
    });
...
export const getCourseById = async (req: Request, res: Response) => {
  ...
  try {
    const course = await prisma.exclusive.findUnique({
      where: { id,
               instructorId,
       },
      include: {
...
export const updateCourse = async (req: Request, res: Response) => {
  ...
  try {
    ...
    const updatedCourse = await prisma.exclusive.update({
      where: { id,
        instructorId,
       },
      data: {
