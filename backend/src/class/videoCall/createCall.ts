...
export const startVideoCall = async (classId: string, userId: string) => {
  const videoCall = await prisma.classVideoCall.create({
    data: {
      classId,
      userId,
      startedAt: new Date(),
...
export const endVideoCall = async (callId: string) => {
  const videoCall = await prisma.classVideoCall.update({
    where: { id: callId },
...
export const joinVideoCall = async (callId: string, userId: string) => {
  const videoCallParticipant = await prisma.classVideoCallParticipant.create({
    data: {
      callId,
      userId,
    },
...
export const leaveVideoCall = async (callId: string, userId: string) => {
  const videoCallParticipant = await prisma.classVideoCallParticipant.deleteMany({
    where: { callId, userId },
  });
